...
!!!abstract "Learning Objectives"
    - Students will be able to explain the meaning of a 
    communication/networking protocol known as HTTP.
    ...
    - Students will be able to develop a web server. using the node.js 
    module http that is based on the HTTP protocol.
...
## 1. JavaScript (Constants, Variables, Controls, Functions, Asynchronous Programming)

???note "LinkedIn Learning activity"
    ...
    - You can also login to LinkedIn Learning via [WCUPA's LinkedIn Learning](https://www.wcupa.edu/LinkedInLearning/) and search 
    for the course title: "Hands-On Introduction: JavaScript".

???note "More Javascript: Callbacks"
...
???tip "Recording: Instructions on Hands-on and Exercises"

???note "Hands-on: test.js"
...
???question "Exercises"
    - Type these programs as separate files and use the command `node` to execute them.

    ???question "Define a function and execute it – Immediately Executing Functions"
        ```js
        (function foo() {
    ...
    ???question "Closure - What is the output of the program? Why?"
        ```js
        function outerFunction(arg) {
        var variableInOuterFunction = arg;
        function bar() {
        console.log(variableInOuterFunction); // Access a variable from
        }
    ...
    ???question "How do we display or add elements from/to an array?"
        ```js
        'use strict'
        var users = [
        {
...
???note "Protocols vs. Applications"
...
???tip "Recording: Instructions on Hands-on "

???note "Lesson 3 – Create a Node.js Module"
...
???note "Lesson 4 – HTTP and Web Servers"
...
???note "Lesson 5 - Handling Incoming Requests"
...
???note "Lesson 7 – Capstone: Create the first web server with HTTP and Node.js"
